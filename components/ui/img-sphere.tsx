    ...
    return (
      ...
      >
        <div className="relative">
          ...
          <button
            onClick={() => setSelectedImage(null)}
            className="absolute w-10 h-10 md:w-12 md:h-12 bg-white hover:bg-gray-100 rounded-full text-gray-800 flex items-center justify-center transition-all shadow-xl hover:scale-110 z-[60]"
          ...
          <div
            className="bg-white rounded-2xl overflow-hidden shadow-2xl flex flex-col mx-4"
          ...
          >
            {/* Image Container */}
            <div className="relative bg-black flex items-center justify-center overflow-hidden" style={{ maxHeight: '60vh' }}>
              <img
            ...
            {/* Text Container with Strong Backdrop Blur and Links */}
            {(selectedImage.title || selectedImage.description) && (
              <div 
                className="bg-white/98 backdrop-blur-xl p-4 md:p-6 border-t border-gray-300 overflow-y-auto shadow-2xl" 
                style={{ maxHeight: '30vh', backdropFilter: 'blur(20px)' }}
              >
                {selectedImage.title && (
                  <h3 className="text-lg md:text-2xl font-semibold mb-2 text-gray-900" style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
                    {selectedImage.title}
                  </h3>
                )}
                {selectedImage.description && (
                  <div>
                    <p className="text-gray-700 leading-relaxed text-sm md:text-base mb-3" style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
                      {selectedImage.description}
                    </p>
                    {selectedImage.link && (
                      <a 
                        ...
                        className="inline-block text-blue-600 hover:text-blue-800 underline text-sm md:text-base font-medium transition-colors"
                        style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}
                        onClick={(e) => e.stopPropagation()}
                      >
                        {selectedImage.linkText || 'Learn more →'}
                      </a>
                    )}
                  </div>
                )}
